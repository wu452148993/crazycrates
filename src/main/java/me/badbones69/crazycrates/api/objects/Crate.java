...
public class Crate {
	...
	private void setDefaultItems(Inventory inv, Player player) {
		for(int i : Arrays.asList(0, 1, 2, 3, 4, 5, 6, 7, 8, 45, 46, 47, 49, 51, 52, 53)) {
			...
			.setMaterial(XMaterial.fromString("STAINED_GLASS_PANE:"+ Preview.getPaneColor(player)))
			//.setMaterial(XMaterial.WHITE_STAINED_GLASS_PANE.parseMaterial())
		...
		if(page == 1) {
			...
			.setMaterial(XMaterial.fromString("STAINED_GLASS_PANE:7"))
			//.setMaterial(XMaterial.WHITE_STAINED_GLASS_PANE.parseMaterial())
			//.setMetaData((short) 7)
			.setName(" ")
			.build());	
